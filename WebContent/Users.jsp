      ...
      <div class="container">
        ...
        <div class="navbar-collapse collapse">
          ...
          <%if(userType==2) {%>
            ...
            <ul class="dropdown-menu">
                <li class="active"><a href="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=user") %>">Users</a></li>
            ...
            <table class="col-md-12 table-bordered table-striped table-condensed cf " style="padding:0px; width: 100%;">
        		...
        		<tbody class="searchable">
        			...
        			for(User user : listOfUsers) {i++;%><tr >
						...
						<a id="button" class="btn btn-primary btn-xs" href="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=updateuser&userId="+user.getId()) %>"><i class="glyphicon glyphicon-cog"></i>&nbsp;Modify</a>
						</div>
						</td>
        			</tr><%} %>
...
<div class="modal fade" id="myModal<%=j %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <form method="post" action="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=dodeleteuser") %>">
