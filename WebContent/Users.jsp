      ...
      <div class="container">
        ...
        <div class="navbar-collapse collapse">
          ...
          <%if(userType==2) {%>
            ...
            <ul class="dropdown-menu">
                <li class="active"><a href="<%= response.encodeUrl(request.getContextPath() + "/CreateUser?action=user") %>">Users</a></li>
...
<div class="container">
    <div class="row">
      ...
      <div class="row">
      	  ...
      	  <a id="btn-create" type="button" class="btn btn-primary" href="<%= response.encodeUrl(request.getContextPath() + "/CreateUser?action=newuser") %>"><i class="glyphicon glyphicon-plus"></i>&nbsp;New User</a>
        </div>
        ...
        <div id="no-more-tables" style="padding:3px"> 		               
            <table class="col-md-12 table-bordered table-striped table-condensed cf " style="padding:0px; width: 100%;">
        		...
        		<tbody class="searchable">
        			<%int i = 0;
        			for(User user : listOfUsers) {i++;%><tr >
						...
						<a id="button" class="btn btn-primary btn-xs" href="<%= response.encodeUrl(request.getContextPath() + "/CreateUser?action=updateuser&userId="+user.getId()) %>"><i class="glyphicon glyphicon-cog"></i>&nbsp;Modify</a>
						</div>
						</td>
        			</tr><%} %>
    ...
    <!-- Modal 1-->
    <%int j=0;
    for(User user:listOfUsers){ j++;%>
<div class="modal fade" id="myModal<%=j %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
  <form method="post" action="<%= response.encodeUrl(request.getContextPath() + "/CreateUser?action=dodeleteuser") %>">
  <input type= "hidden" name="userid" value="<%=user.getId()%>"></input>
    <div class="modal-content">
      ...
      <h4><span class="glyphicon glyphicon-exclamation-sign"></span>&nbsp;Are you sure you want to delete the User: <b><%=user.getUserName()%></b> ?</h4>
      </div>
