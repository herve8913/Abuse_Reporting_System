            ...
            <table class="col-md-12 table-bordered table-striped table-condensed cf " style="padding:0px; width: 100%;">
        		...
        		for (AbuseReport abuseReport:userAbuseReportView){ i++; %>
        			<tr>
        				...
        				<td data-title="Status"><%if(abuseReport.getStatus()==1){ %>Saved<%}else if(abuseReport.getStatus()==2){ %>Submitted<%}else if (abuseReport.getStatus()==3){%>Approved<%}else if(abuseReport.getStatus()==4){ %>Investigated<%}else if(abuseReport.getStatus()==5){ %>Decision Received<%}else if(abuseReport.getStatus()==6){ %>Appealed<%}%></td>
